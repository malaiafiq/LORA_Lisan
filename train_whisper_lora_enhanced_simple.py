...
class ExecutionTimer:
    ...
    def end_step(self):
        ...
        if self.current_step and self.step_start:
            ...
            print(f"[DONE] {self.current_step}")
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        print("PyTorch is NOT using the GPU (CPU training will be slower)")
    timer.end_step()
    ...
    if torch.cuda.is_available():
        ...
        print(f"Estimated training time: {estimated_total_time:.1f} minutes (GPU)")
    else:
        estimated_time_per_batch = 10  # seconds (rough estimate for CPU)
        estimated_total_time = (total_batches * estimated_time_per_batch) / 60  # minutes
        print(f"Estimated training time: {estimated_total_time:.1f} minutes (CPU)")
